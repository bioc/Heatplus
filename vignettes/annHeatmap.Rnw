...
Figure~\ref{fig6a} shows a similar plot as Figure~\ref{fig6}, but cut at a distiance of 7500 (resulting in two instead of three clusters) and with costumised cluster labels. 
...
ann4b = annHeatmap2(exprs(exdat2),
  ann=list(inclRef=FALSE, Col=list(data=pData(exdat2)), Row=list(data=annFeatures)),
  labels=list(Row=list(nrow=7), Col=list(nrow=2)))
